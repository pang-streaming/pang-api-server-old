...
public interface FollowDocuments {
    ...
    DataResponse<List<FollowingResponse>> getFollowings(String username);

    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "팔로워 조회 API", description = "자신을 팔로워를 가져옵니다.")
    DataResponse<List<FollowingResponse>> getFollowers(String username);

    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "팔로우/언팔로우 API", description = "팔로우하거나 언팔로우합니다.")
    Response followOrUnfollow(String username);
}